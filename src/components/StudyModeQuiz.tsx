...
export const StudyModeQuiz: React.FC<StudyModeQuizProps> = ({
  ...
  useEffect(() => {
    ...
    const allOptions = [currentCard.german, ...distractors].sort(
      () => Math.random() - 0.5,
    );
  ...
  const handleCheckTypedAnswer = (e: React.FormEvent) => {
    ...
    const normalize = (str: string) =>
      str.trim().toLowerCase().normalize("NFC").replace(/\s+/g, " ");

    const normalizedInput = normalize(typedAnswer);
    const normalizedTarget = normalize(currentCard.german);
    const targetWithArticle = currentCard.article
      ? normalize(`${currentCard.article} ${currentCard.german}`)
      : null;
          ...
          {!isAnswered && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">
                {getTranslation(uiLanguage, "germanCharacters")}
              </span>
